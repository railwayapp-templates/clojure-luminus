...
The `home-routes` handler in the `brapp.routes.home` namespace
...
The `home-page` function will in turn call the `brapp.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `brapp.handler` namespace:
...
Request middleware functions are located under the `brapp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `brapp.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `brapp.db.core` in some other namespace.
